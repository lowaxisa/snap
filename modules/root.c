...
cl_routines routines[MAX_COROUTINES];
...
void routine() {
    ...
    routines[scl_current_coroutine_pid].status = scl_coroutines[scl_current_coroutine_pid].status;
                ...
                case 2:
                    ...
                    for (size_t i = 0; i < MAX_COROUTINES; i++) {
                        if (routines[i].id != msg->sender_id && i == msg->pid && scl_coroutine_find(msg->pid)) {
                            ...
                            routines[i].status = scl_coroutines[msg->pid].status;
                    ...
                    break;
                case 3:
                    scl_coroutine_send(msg->pid, 3, &routines);
                    break;
            };
        }
